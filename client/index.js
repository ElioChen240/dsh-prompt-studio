// DSH Prompt Studio Client Plugin (Vanilla React.createElement without JSX/Bundler)
  ...
  apply(ctx) {
    ...
    slots.inject('settings.section', () => {
      return slots.register({
        ...
        return React.createElement(PromptStudioView, { ctx });
      });
    });
  }
};

function PromptStudioView({ ctx }) {
  const [state, setState] = React.useState(null);
  const [activeTab, setActiveTab] = React.useState('editor'); // editor | versions | benchmark
  ...
  return React.createElement('div', {
    ...
    // TAB 3: Golden Benchmark & Regression
