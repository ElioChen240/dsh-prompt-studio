export class EvaluatorEngine {
     ...
     */
    evaluateOutput(testCase, output, latencyMs) {
        let score = 100;
        const feedbackList = [];
        // 1. 检查命中预期关键字
        const missingExpected = testCase.expectedKeywords.filter(kw => !output.toLowerCase().includes(kw.toLowerCase()));
        ...
        // 2. 检查违规/不应出现的关键词
        const hitForbidden = testCase.forbiddenKeywords.filter(kw => output.toLowerCase().includes(kw.toLowerCase()));
